...
const Messages = ({ user }) => {
    ...
    const { data: messagesContent, loading: loadingMessages } = useSubscription(MESSAGES_SUBSCRIPTION);
    const [messages, setMessages] = useState([]);

    useEffect(() => {
        if (messagesContent) {
            setMessages(prevMessages => [...prevMessages, messagesContent.messageAdded]);
        }
    }, [messagesContent]);
    ...
    return (
        <div className="message-container">
            {messages.map(message =>
                <div key={message.id} className={message.user === user ? "message_me-block" : "message-block"}>
                    {message.user !== user &&
...
const Chat = () => {
    const [user, setUser] = useState("Blya1");
